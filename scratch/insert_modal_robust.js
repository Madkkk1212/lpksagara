...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('✓ Buka Akses')) {
    // Found the button, let's find the closing )} of the modal which should be a few lines below
...
if (targetIdx !== -1) {
  ...
  lines.splice(targetIdx + 1, 0, modalContent);
  const updatedContent = lines.join('\n');
  fs.writeFileSync(filePath, updatedContent, 'utf8');
  console.log('QuizAccessManager.tsx monitoring modal successfully appended via line index splicing.');
} else {
  console.error('Error: Could not locate appropriate line-splice index!');
}
