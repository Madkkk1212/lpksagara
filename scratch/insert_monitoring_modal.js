...
// Find the duration modal block ending specifically
const targetPattern = '              </div>\r\n           </div>\r\n         </div>\r\n       )}';
const targetPatternLF = '              </div>\n           </div>\n         </div>\n       )}';
...
if (content.includes(targetPattern)) {
  content = content.replace(targetPattern, modalContent);
  fs.writeFileSync(filePath, content, 'utf8');
  console.log('QuizAccessManager.tsx monitoring modal updated successfully (CRLF).');
} else if (content.includes(targetPatternLF)) {
  content = content.replace(targetPatternLF, modalContent);
  fs.writeFileSync(filePath, content, 'utf8');
  console.log('QuizAccessManager.tsx monitoring modal updated successfully (LF).');
} else {
  console.error('Error: Duration modal block end pattern not found!');
}
