...
export default function AllStudentsAssessment({ students, levels }: { students: Profile[], levels: StudyLevel[] }) {
  ...
  // Establish Supabase Realtime Subscription for assessment grades
  useEffect(() => {
    if (!selectedBatchId || !selectedLevelId || templates.length === 0) return;

    const channel = supabase
      .channel("realtime-grades-assessment")
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "assessment_chapter_grades"
        },
        () => {
          // Auto-sync when database changes occur
          fetchGradesForBatch();
        }
      )
      .subscribe();
