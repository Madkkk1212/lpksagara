  ...
  const handleReorderCategories = async (newOrder: MaterialCategory[]) => {
    ...
    try {
      const updates = newOrder.map((cat, index) => 
        upsertMaterialCategory({ ...cat, sort_order: index + 1 })
      );
      await Promise.all(updates);
    } catch (e) { console.error(e); }
  };

  const handleReorderLevels = async (newOrder: StudyLevel[]) => {
    setLevels(newOrder);
    try {
      const updates = newOrder.map((lvl, index) => 
        upsertStudyLevel({ ...lvl, sort_order: index + 1 })
      );
      await Promise.all(updates);
    } catch (e) { console.error(e); }
  };

  const handleReorderChapters = async (newOrder: StudyChapter[]) => {
    setChapters(newOrder);
    try {
      const updates = newOrder.map((chap, index) => 
        upsertStudyChapter({ ...chap, sort_order: index + 1 })
      );
      await Promise.all(updates);
    } catch (e) { console.error(e); }
  };

  const handleReorderMaterials = async (newOrder: StudyMaterial[]) => {
    setMaterials(newOrder);
    try {
      const updates = newOrder.map((mat, index) => 
        upsertStudyMaterial({ ...mat, sort_order: index + 1 })
      );
      await Promise.all(updates);
    } catch (e) { console.error(e); }
  };
            ...
            {appCategories.map(cat => (
               ...
               >
                 <div 
                    ...
                    className={`px-8 py-6 rounded-[2rem] font-black flex items-center gap-6 shadow-xl transition-all border border-white/10 h-full ${selectedCategoryFilter === cat.id ? 'bg-teal-500 ring-4 ring-teal-500/20' : 'bg-white/5 hover:bg-white/10'}`}
                 >
                    {cat.icon_url ? (
                      <div className="h-16 w-16 rounded-2xl bg-white flex items-center justify-center p-2.5 shadow-sm shrink-0">
                        <img src={cat.icon_url || undefined} alt="icon" className="w-full h-full object-contain" />
                      </div>
                    ) : (
                      <div className="h-16 w-16 rounded-2xl bg-white/10 flex items-center justify-center text-3xl shadow-sm shrink-0">
                        🌟
                      </div>
                    )}
                    <div className="text-left">
                       <h4 className="text-xl leading-tight">{cat.name}</h4>
                       <p className="text-[10px] font-medium text-white/50 mt-1 line-clamp-1">{cat.description || 'Belum ada deskripsi.'}</p>
                    </div>
                 </div>
...
<<<<<<< HEAD
           <Reorder.Group 
              ...
              onReorder={(newOrder) => {
                // Merge reordered items back into the main levels state
                const otherLevels = levels.filter(l => l.category_id !== selectedCategoryFilter);
                handleReorderLevels([...otherLevels, ...newOrder]);
              }}
