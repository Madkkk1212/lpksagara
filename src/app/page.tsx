...
=======
import { getTheme, getBanners, getMaterialCategories, getMaterials, getExamLevels, getProfileByEmail, upsertProfile, getStudyLevels, getCompletedMaterials, getTotalStudyMaterialsCount, getUserLastProgressDetails, getStudyChapters } from "@/lib/db";
import { AppTheme, BannerSlide, MaterialCategory, Material, ExamLevel, Profile, StudyLevel, StudyChapter } from "@/lib/types";
...
=======
          ...
          <aside className="hidden lg:block space-y-8 animate-in fade-in slide-in-from-right-4 duration-1000">
             <div className="rounded-[3rem] bg-slate-900 p-10 text-white shadow-2xl">
                <h4 className="text-xl font-black italic mb-6">Target Minggu Ini</h4>
                <div className="space-y-6">
                   {[{ label: "Vocabulary N5", progress: 85 }, { label: "Reading Test #3", progress: 40 }].map(task => (
                     <div key={task.label}>
                        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-3"> <span>{task.label}</span> <span className="text-teal-400">{task.progress}%</span> </div>
                        <div className="h-1.5 bg-white/10 rounded-full overflow-hidden"> <div className="h-full bg-teal-400 transition-all duration-1000" style={{ width: `${task.progress}%` }} /> </div>
                     </div>
                   ))}
                </div>
             </div>
