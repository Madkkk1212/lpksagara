...
<<<<<<< HEAD
              <section className="space-y-8">
                <div className="text-center mb-4 pt-4">
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Lengkapi data profil Anda untuk pengalaman terbaik</p>
                </div>
                ...
                <div className="bg-white rounded-[3rem] border border-slate-100 p-8 md:p-12 shadow-sm space-y-10">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Nama Lengkap</label>
                         <input 
                            type="text" 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all"
                      ...
                      {/* Nama Panggilan */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Nama Panggilan</label>
                         <input 
                            type="text" 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all"
                            value={coreData.nickname}
                            onChange={(e) => setCoreData({...coreData, nickname: e.target.value})}
                         />
                      </div>

                      {/* Jenis Kelamin */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Jenis Kelamin</label>
                         <select 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all appearance-none cursor-pointer"
                            value={coreData.gender}
                            onChange={(e) => setCoreData({...coreData, gender: e.target.value as any})}
                         >
                      ...
                      {/* Nomor HP */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Nomor HP / WA</label>
                         <input 
                            type="tel" 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all"
                            value={coreData.phone}
                            onChange={(e) => setCoreData({...coreData, phone: e.target.value})}
                         />
                      </div>

                      {/* Tanggal Lahir */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Tanggal Lahir</label>
                         <input 
                            type="date" 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all"
                            value={coreData.birth_date}
                            onChange={(e) => setCoreData({...coreData, birth_date: e.target.value})}
                         />
                      </div>

                      {/* Institusi */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Asal Institusi / Sekolah</label>
                         <input 
                            type="text" 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all"
                            value={coreData.institution}
                            onChange={(e) => setCoreData({...coreData, institution: e.target.value})}
                         />
                      </div>

                      {/* Target Level */}
                      <div className="space-y-3">
                         <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Target Sertifikasi (JLPT)</label>
                         <select 
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all appearance-none cursor-pointer"
                            value={coreData.target_level}
                            onChange={(e) => setCoreData({...coreData, target_level: e.target.value})}
                         >
                            <option value="">Pilih Target...</option>
                            <option value="N5">JLPT N5 (Dasar)</option>
                            <option value="N4">JLPT N4 (Menengah Bawah)</option>
                            <option value="N3">JLPT N3 (Menengah)</option>
                            <option value="N2">JLPT N2 (Menengah Atas)</option>
                            <option value="N1">JLPT N1 (Mahir)</option>
                         </select>
                      </div>
                   </div>
                   ...
                   {/* Alamat */}
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-4">Alamat Lengkap</label>
                      <textarea 
                         rows={3}
                         className="w-full bg-slate-50 border border-slate-100 rounded-[2rem] px-8 py-6 font-bold text-slate-800 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all resize-none"
                         value={coreData.address}
                         onChange={(e) => setCoreData({...coreData, address: e.target.value})}
                      />
                   </div>
                   ...
                   <div className="pt-6 border-t border-slate-50">
                      <button 
                         ...
                         disabled={loading}
                         className="w-full md:w-auto px-12 py-5 bg-slate-900 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.3em] shadow-2xl active:scale-95 transition-all disabled:opacity-50"
                      >
                ...
                {/* Dynamic Fields Section (if any) */}
                {fields.filter(f => f.type !== 'file').length > 0 && (
                  <div className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm space-y-8">
                    <h5 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 text-center">Data Tambahan Kustom</h5>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
