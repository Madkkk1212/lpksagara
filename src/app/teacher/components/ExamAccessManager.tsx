...
import { Profile, ExamLevel, ExamTest } from "@/lib/types";
import { getExamLevels, getExamTests, getStudentBatches } from "@/lib/db";
import { supabase } from "@/lib/supabase";
import { BookOpen, Search, Zap, X, Trophy } from "lucide-react";

interface ExamAccessManagerProps {
  teacher: Profile;
}

export default function ExamAccessManager({ teacher }: ExamAccessManagerProps) {
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      const [allLevels, allBatches, profilesResult] = await Promise.all([
        getExamLevels(),
        getStudentBatches(),
        supabase.from('profiles').select('*').neq('is_teacher', true).neq('is_admin', true)
      ]);

      setLevels(allLevels.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)));
      ...
      // Fetch all tests
      const allTestsRequests = allLevels.map(l => getExamTests(l.id));
      const testsResults = await Promise.all(allTestsRequests);
      const flattenedTests = testsResults.flat();
  ...
  const toggleExamAccess = async (testId: string, type: 'batch' | 'student', identifiers: string[], currentStatus: boolean) => {
    try {
      ...
      const requests = identifiers.map(id => {
        ...
        return supabase.from('exam_access_controls').upsert(payload, { 
          onConflict: type === 'batch' ? 'batch,test_id' : 'student_id,test_id' 
        });
      });
      ...
      alert("Gagal mengubah akses exam: " + err.message);
    }
  };

  const isExamActive = (testId: string, batch: string, studentIds: string[]) => {
    if (studentIds.length > 0) {
      const activeCount = studentIds.filter(sid => {
        const control = accessControls.find(c => c.student_id === sid && c.test_id === testId);
    ...
    const control = accessControls.find(c => c.batch === batch && c.test_id === testId);
    return control ? control.is_active : false;
  };
  ...
  if (loading) {
    return <div className="p-20 text-center text-slate-400 font-black uppercase tracking-widest text-xs animate-pulse">Loading Exam Controls...</div>;
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-slate-900 italic tracking-tight uppercase leading-none">Kontrol Akses Exam</h2>
          <p className="text-xs text-slate-500 font-medium">Buka Ujian Sertifikasi (Exam) secara real-time untuk Batch atau Siswa.</p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-6">
          ...
          <div className="flex flex-col gap-2 w-72">
            <label className="text-[9px] font-black uppercase text-slate-400 ml-2">Cari Judul Exam</label>
            <div className="relative">
      ...
      <div className="space-y-6">
        {levels.map(lvl => {
          const levelTests = tests.filter(t => t.level_id === lvl.id && t.title.toLowerCase().includes(searchTerm.toLowerCase()));
          if (levelTests.length === 0) return null;
          
          const isExpanded = expandedLevels.includes(lvl.id);
          
          return (
            <div key={lvl.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden transition-all duration-500 hover:shadow-md">
               <button 
                 onClick={() => toggleLevel(lvl.id)}
                 className="w-full px-10 py-10 flex items-center justify-between hover:bg-slate-50 transition-colors text-left"
               >
                  <div className="flex items-center gap-8">
                    <div className="h-16 w-16 bg-indigo-600 text-white rounded-3xl flex items-center justify-center text-2xl shadow-xl shadow-indigo-900/10 font-black italic">
                      {lvl.level_code.toUpperCase()}
                    </div>
                    <div>
                      <h3 className="text-xl font-black text-slate-900 italic tracking-tight leading-none mb-2">{lvl.title}</h3>
                      <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">{levelTests.length} Exam Tests</p>
                    </div>
                  </div>
               ...
               {isExpanded && (
                 <div className="px-10 pb-12 pt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in slide-in-from-top-6 duration-700">
                    {levelTests.map(test => {
                      const active = isExamActive(test.id, selectedBatch, selectedStudentIds);
                      const isPartial = active === 'partial';
                      
                      return (
                        <div key={test.id} className={`group p-8 rounded-[2.5rem] border transition-all duration-500 ${active === true ? 'bg-emerald-50 border-emerald-200' : isPartial ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-100'}`}>
                           <div className="flex items-center justify-between mb-8">
                              ...
                              <div className={`h-10 w-10 rounded-2xl flex items-center justify-center transition-all ${active === true ? 'bg-emerald-100 text-emerald-600' : isPartial ? 'bg-amber-100 text-amber-600' : 'bg-white text-slate-300 shadow-sm'}`}>
                                <Trophy size={20} className={active ? 'animate-pulse' : ''} />
                              </div>
                           </div>
                           
                           <h4 className="text-base font-black text-slate-800 mb-8 leading-tight min-h-[48px] group-hover:text-indigo-600 transition-colors">{test.title}</h4>
                           
                           <button 
                             onClick={() => toggleExamAccess(
                               test.id, 
